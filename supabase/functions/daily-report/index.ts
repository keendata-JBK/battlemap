...
Deno.serve(async (request) => {
  ...
  if (dataError) return jsonResponse({ error: `项目或销售目录读取失败：${dataError.message}` }, 500);
...
3. activityType 只能是 call、meeting、visit、proposal、task、note。明确“拜访/到访/现场”用 visit；明确会议或交流会用 meeting；电话/微信沟通用 call；方案、标书、汇报材料用 proposal。
...
{"entries":[{"reportDate":"YYYY-MM-DD","salespersonId":"uuid或null","salespersonName":"姓名或空","projectId":"uuid或null","projectName":"项目名或空","activityType":"visit","content":"事实描述","customerContact":"客户联系人或空","matchConfidence":0.95,"matchReason":"匹配依据","rawSegment":"对应原文"}],"warnings":["需要管理员注意的问题"]}`;
  ...
  try {
    ...
    return jsonResponse({ error: `日报识别结果解析失败：${error instanceof Error ? error.message : "未知错误"}` }, 502);
  }
  ...
  const entries = (Array.isArray(parsed.entries) ? parsed.entries : []).slice(0, 200).map((entry: Record<string, unknown>, index: number) => {
