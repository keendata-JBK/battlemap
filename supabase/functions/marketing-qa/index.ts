...
async function processJob(
  ...
  try {
    await adminClient.from("marketing_qa_jobs").update({ status: "processing", started_at: new Date().toISOString(), error_message: null }).eq("id", jobId);
    ...
    if (dataError) throw new Error(`营销数据读取失败：${dataError.message}`);
    ...
    const dataScope = profileResult.data?.role === "sales" ? "本人负责项目" : "全部可见项目";
    const dataSnapshot = {
    ...
    const { error: updateError } = await adminClient.from("marketing_qa_jobs").update({
      status: "completed",
      answer,
      error_message: null,
      model: "gpt-5.5",
...
Deno.serve(async (request) => {
  ...
  if ((body.action ?? "create") !== "create") return jsonResponse({ error: "Unsupported action" }, 400);
  const question = String(body.question ?? "").trim().slice(0, 4000);
  if (!question) return jsonResponse({ error: "请输入要查询的问题" }, 400);
  const history = safeHistory(body.history);

  const { data: job, error: insertError } = await callerClient.from("marketing_qa_jobs").insert({
  ...
  const task = processJob(job.id, user.id, callerClient, adminClient, question, history, gatewayBaseUrl, gatewayKey);
