...
const headers = "项目名称,客户主体,关键联系人,联系人手机号,联系人邮箱,项目类型,经营区域,省份,城市,区县,行政区划代码,经度,纬度,金额（万元）,负责人,售前负责人,销售阶段,优先级,下一步动作,计划日期,预计成交日期,牵线单位";

describe("parseImportCsv", () => {
  it("accepts Beijing projects and blank lead amounts", () => {
    const csv = `${headers}\n北京项目,北京客户,,,,政府资源,北京区域,北京,北京,海淀区,110108,116.298056,39.959912,,测试用户,,商机,P2,,,,北京市经信局`;
  ...
  it("normalizes other Chinese provinces to other region", () => {
    const csv = `${headers}\n广州项目,广州客户,,,,政府资源,华南区域,广东,广州,天河区,440106,113.3612,23.1247,100,测试用户,,线索,P2,,,,广州市政数局`;
    const [row] = parseImportCsv(csv);
    expect(row.status).toBe("通过");
  ...
  it("rejects unsupported region values when the adcode cannot be classified", () => {
    const csv = `${headers}\n未知项目,未知客户,,,,政府资源,华南区域,未知省,未知市,未知区,990106,113.3612,23.1247,100,测试用户,,线索,P2,,,,测试单位`;
    const [row] = parseImportCsv(csv);
  ...
  it("requires a referral unit for government-resource projects", () => {
    const csv = `${headers}\n政府项目,政府客户,,,,政府资源,西南区域,四川,成都,金牛区,510106,104.043487,30.692058,300,测试用户,,商机,P1,,,,`;
    const [row] = parseImportCsv(csv);
    expect(row.status).toBe("需修正");
    expect(row.error).toContain("政府资源项目需填写牵线单位");
  });
});
